...
import { capitalizeFirstLetter, identifierOfTheme } from '../utils/generic';
...
export abstract class Component {
    ...
    async getAttributeText(name: string, locator: Locator): Promise<string> {
        const attrValue = await locator.getAttribute(name);
        return attrValue || '';
    }

    async validateTheme( locatorProps: LocatorProps = {}, mode: string): Promise<void> {
        await test.step(`Validate theme  ${this.typeOf} changed with name "${this.componentName}"`, async () => {
            const locator = this.getLocator(locatorProps);
            let attrValue = await this.getAttributeText('title', locator)
            expect(identifierOfTheme(attrValue, mode)).toEqual(true);
        });
    }
}